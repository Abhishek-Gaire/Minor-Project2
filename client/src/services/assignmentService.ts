...
export const assignmentService = {
  ...
  async getAssignments(
    ...
    try {
      ...
      if (grade) {
        const numberGrade = grade.split("")[1]; // you might want to validate this
        params.grade = numberGrade;
      }

      const response = await axios.get(
        `${API_BASE_URL}/api/v1/assignments/get`,
        {
          params,
        }
      );
  ...
  async getAssignmentById(id: number) {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/api/v1/assignments/get/${id}`
      );
      return response.data;
    } catch (error) {
  ...
  async createAssignment(
    assignmentData: CreateAssignmentForm,
    teacherName: string
  ) {
    try {
  ...
  async updateAssignment(id: number, assignmentData: Partial<Assignment>) {
    try {
      const response = await axios.put(
        `${API_BASE_URL}/api/v1/assignments/update/${id}`,
        assignmentData,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  async deleteAssignment(id: number) {
    try {
      const response = await axios.delete(
        `${API_BASE_URL}/api/v1/assignments/delete/${id}`
      );
      return response.data;
    } catch (error) {
  ...
  async submitAssignment(submissionData: SubmissionForm, file: File) {
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/api/v1/assignments/submit`,
        formData,
        {
          headers: {
  ...
  async getSubmissions(assignmentId: number) {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/api/v1/assignments/submissions/${assignmentId}`
      );
      return response.data;
    } catch (error) {
      console.error(
  ...
  async gradeSubmission(
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/api/v1/assignments/grade/${assignmentId}`,
        {
          studentId,
          grade,
          feedback,
